...
export function TeamPanel({ currentState, playerId, onSwitchTeam }: TeamPanelProps) {
    ...
    return (
        <div className="bg-card-bg p-6 rounded-xl shadow-lg border border-gray-200 mt-4">
                    ...
                    return (
                        <div key={team.id} className={`p-3 rounded-lg border-2 ${isMyTeam ? 'bg-gray-50' : ''}`} style={{ borderColor: teamColor }}>
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className="font-mono text-xl font-bold" style={{ color: teamColor }}>
                                    {team.score}
                                </span>
                            </div>
                            <ul className="flex flex-col gap-1 mb-3">
                                {team.playerIds.map((pid) => {
                                    ...
                                    return (
                                        <li key={pid} className={`text-sm ${isMe ? 'font-bold text-black' : 'text-gray-600'}`}>
